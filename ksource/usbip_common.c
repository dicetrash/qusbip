...
void usbip_names_get_class(char *buff, size_t size, uint8_t space,
			   uint8_t subclass, uint8_t protocol)
{
	const char *c, *s, *p;

    if (space == 0 && subclass == 0 && protocol == 0) {
        snprintf(buff, size, "(Defined at Interface level) (%02x/%02x/%02x)", space, subclass, protocol);
		return;
	}

    p = names_protocol(space, subclass, protocol);
	if (!p)
		p = "unknown protocol";

    s = names_subclass(space, subclass);
	if (!s)
		s = "unknown subclass";
    ...
    snprintf(buff, size, "%s / %s / %s (%02x/%02x/%02x)", c, s, p, space, subclass, protocol);
}
